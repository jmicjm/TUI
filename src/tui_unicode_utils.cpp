...
namespace tui
{
	...
	{
		struct grapheme_range_info { unsigned int r_s, r_e; GRAPHEME_TYPE r_t; };
	...
	std::u32string utf8ToUtf32(const char* utf8_str, size_t len, bool shrink_after, float reserve_ratio)
	{
	...
	std::u32string utf8ToUtf32(const std::string& utf8_str, bool shrink_after, float reserve_ratio)
	{
		return utf8ToUtf32(utf8_str.data(), utf8_str.size(), shrink_after, reserve_ratio);
	}

	std::string utf32ToUtf8(const char32_t* utf32_str, size_t len, bool shrink_after, float reserve_ratio)
	{
		std::string utf8_str;
	...
	std::string utf32ToUtf8(const std::u32string& utf32_str, bool shrink_after, float reserve_ratio)
	{
	...
	{
		GRAPHEME_TYPE l_t = getGraphemeType(l);
		GRAPHEME_TYPE r_t = getGraphemeType(r);
		...
		*/
		...
		//GB7
		if ((l_t == GRAPHEME_TYPE::LV || l_t == GRAPHEME_TYPE::V) 
			&&
			(r_t == GRAPHEME_TYPE::V || r_t == GRAPHEME_TYPE::T)
			)
		{
			return false;
		}
		//GB8
		if ((l_t == GRAPHEME_TYPE::LVT || l_t == GRAPHEME_TYPE::T)
			&&
			r_t == GRAPHEME_TYPE::T
			)
		{
		...
		//GB9
		if (r_t == GRAPHEME_TYPE::EXTEND || r_t == GRAPHEME_TYPE::ZWJ)
		{
			return false;
		}
		//GB9a
